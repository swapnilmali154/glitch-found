...
import { Breadcrumbs } from '../App.js';
...
const Master = () => {
        ...
        if (activeButton === 1) {
            try {
                ...
                if (issueTypeObj.issueType !== "") {
                    setLoading(true);
                    const result = await postData(ADD_NEW_TYPE, issueTypeObj);
                    if (result !== undefined) {
                        setLoading(false);
                        await notify(result.message, 'success');
                        //debugger
                        resetIssueType();
                        getIssueTypeList();
                        resetIssueObj();
            ...
            if (issueObj.status !== '' && issueObj.orderNo !== 0 && issueObj.isActive != false) {
                try {
                    setLoading(true)
                    const result = await postData(ADD_NEW_STATUS, issueObj);
                    if (result !== undefined) {
                        setLoading(false);
                        await notify(result.message, 'success');
                        //debugger
    ...
    const update = async () => {
        if (activeButton === 1) {
            try {
                setLoading(true);
                const result = await postData(UPDATE_ISSUE_TYPE, issueTypeObj);
                if (result !== undefined) {
                    setLoading(false);
                    await notify(result.message, 'success');
                    //debugger
            ...
            try {
                setLoading(true);
                const result = await postData(UPDATE_STATUS, issueObj);
                if (result !== undefined) {
                    setLoading(false);
                    await notify(result.message, 'success');
                    //debugger
    ...
    const onDelete = async (data) => {
        ...
        if (data.statusId) {
            try {
                ...
                const result = await deleteData(DELETE_STATUS_BY_ID, data.statusId);
                if (result !== undefined) {
            ...
            try {
                ...
                const result = await deleteData(DELETE_ISSUE_TYPE_BY_ID, data.issueTypeId);
                if (result !== undefined) {
                    await notify(result.message, 'success');
                    //debugger
                    getIssueTypeList();
    ...
    const handleButtonClick = async (buttonId) => {
        ...
        switch (buttonId) {
            ...
            case 2:
                // getissueSatusList();
                try {
                    // const issueStatusList = await getissueSatusList(); 
                    setColumnDefs([
                        {
                            headerName: "Sr.No",
                            field: 'serialNumber',
